...
def step_file_path():
    ...
    return os.path.join(current_dir, "test_files", "Part_Smiely_holes_only.STEP")

@pytest.fixture
def custom_step_file_path(request):
    """获取自定义测试用的STEP文件路径"""
    current_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(current_dir, "test_files", request.param)
...
@pytest.fixture
def custom_loaded_shape(custom_step_file_path):
    """加载自定义STEP文件并返回形状对象"""
    reader = StepReader()
    shape = reader.read_file(custom_step_file_path)
    assert isinstance(shape, TopoDS_Shape), "加载的对象不是TopoDS_Shape类型"
    return shape
...
@pytest.mark.parametrize(
    "custom_step_file_path,expected_holes,expected_slots,expected_bosses",
    [
        ("Part_Smiely_holes_only.STEP", 5, 0, 0),  # 只有孔的测试文件
        # 可以添加更多测试文件和期望的特征数量
        # ("another_test_file.STEP", 2, 3, 1),  # 包含2个孔、3个槽、1个凸台
    ],
    indirect=["custom_step_file_path"]
)
def test_feature_count(custom_loaded_shape, feature_recognizer, expected_holes, expected_slots, expected_bosses):
    """测试STEP文件中特征的数量"""
    # 识别特征
    features = feature_recognizer.recognize(custom_loaded_shape)

    # 验证特征字典结构
    assert 'holes' in features, "特征中缺少holes键"
    assert 'slots' in features, "特征中缺少slots键"
    assert 'bosses' in features, "特征中缺少bosses键"
    ...
    print(f"- 孔: {len(holes)}")
    print(f"- 槽: {len(slots)}")
    print(f"- 凸台: {len(bosses)}")

    # 断言特征数量符合预期
    assert len(holes) == expected_holes, f"应该识别出{expected_holes}个孔，但实际识别出 {len(holes)} 个"
    assert len(slots) == expected_slots, f"应该识别出{expected_slots}个槽，但实际识别出 {len(slots)} 个"
    assert len(bosses) == expected_bosses, f"应该识别出{expected_bosses}个凸台，但实际识别出 {len(bosses)} 个"