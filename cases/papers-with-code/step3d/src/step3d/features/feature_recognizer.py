...
class FeatureRecognizer:
    ...
    def _recognize_holes(self, shape: TopoDS_Shape) -> List[Dict[str, Any]]:
        ...
        for face_set in faces:
            ...
            hole_feature = {
                ...
                'radius': params['radius'],  # 直接添加radius
    ...
    def _is_hole_feature(self, faces: Set[TopoDS_Face]) -> bool:
        ...
        # 检查是否是内部特征（孔）
        if not self._is_internal_feature(cylinder_faces[0]):
            return False
        ...
        if plane_faces:
            for plane_face in plane_faces:
                ...
                normal = surface.Plane().Axis().Direction()
                # 平面应该大致垂直于圆柱轴向
                if abs(abs(normal.Dot(axis)) - 1.0) > 0.2:  # 允许20%的误差
                    return False
    ...
    def _is_internal_feature(self, face: TopoDS_Face) -> bool:
        """判断是否是内部特征（通过检查法向量方向）"""
        from OCC.Core.BRepGProp import brepgprop_NormalProperties
        from OCC.Core.GProp import GProp_GProps
        
        props = GProp_GProps()
        brepgprop_NormalProperties(face, props)
